...
const create = async (req, res) => {
  try {
    const { type, supportType, description, student, sendTo } = req.body;
    if (
      (isEmpty(type),
      isEmpty(supportType) ||
        isEmpty(description) ||
...
const getAll = async (req, res) => {
  try {
    ...
    let allRaiseATicket = await raiseATicket.find({ raiseType: type });
    if (
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json([
          {
            msg: "Ticket Status updated successflly",
            data: studentData,
            res: "success",
          },
        ]);
